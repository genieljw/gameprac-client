...
export default class PlayingScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 첫 번째 타일 스프라이트
    this.tile1 = this.add.tileSprite(0, 0, Config.width, Config.height, "background").setOrigin(0, 0);

    // 두 번째 타일 스프라이트 (오른쪽에 이어붙임)
    this.tile2 = this.add.tileSprite(Config.width, 0, Config.width, Config.height, "background").setOrigin(0, 0);

    // 월드 크기를 타일 두 개의 크기로 확장
  ...
  setupSocketListeners() {
    this.socket.on('state', (state) => {
      Object.keys(state).forEach((playerId) => {
        ...
        //const bullets = state[];
        // bullet 하나 생성시 -> 모든 player sprite에 대해 collider를 add해야 함
    ...
    this.socket.on('shootBeam', (data)=>{
      if (data.playerId != this.m_player.id) {    //수신된 발사체 정보가 현재 플레이어가 아닐 때만 화면에 표시(이중 출력 방지)
        const otherbeam = this.createBeam(data.x, data.y, {x: data.targetX, y: data.targetY});
        
        this.physics.add.collider(otherbeam, this.otherPlayersGroup, (otherbeam, player) => {
          console.log(`Player ${player.id} hit by beam!`);
          otherbeam.destroy();
        });

        this.physics.add.collider(otherbeam, this.m_player, (otherbeam) => {
          console.log(`You were hit by a beam!`);
          otherbeam.destroy();
        });
